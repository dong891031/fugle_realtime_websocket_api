...
#         if __name__ == "__main__":
        ...
        try:
            ...
            if symbol_id != symbol:
                ...
                while [i for i in threading.enumerate() if i.name == symbol+'chart'] != []:
                    ...
                    [i for i in threading.enumerate() if i.name == symbol +
                     'chart'][0].kill()

                    time.sleep(1)

                print('kill complete！')
                t = create_Thread(target=self.chart_websocket, args=[
                                  symbol_id], name=symbol_id+'chart')
                t.start()
                time.sleep(2)
    ...
    def get_chart_data(self, n, symbol_id):
        ...
        df['at'] = pd.to_datetime(df['at']).dt.tz_localize(
            None) + datetime.timedelta(hours=8)
        df = df.set_index('at')
        ...
        df_ohlc['at'] = df_ohlc['at'].apply(
            lambda x: x if x <= close_time else close_time)
...
class quote_websocket_api():
    ...
    def get_quote_msg(self, symbol_id):
        ...
        try:
            ...
            if symbol_id != symbol:
                ...
                print('kill complete！')
                t = create_Thread(target=self.quote_websocket, args=[
                                  symbol_id], name=symbol_id+'quote')
                t.start()
    ...
    def get_first_quote_data(self, message):
        ...
        ask.reverse()
        df_ask = pd.DataFrame(ask, columns=['price', 'unit']).rename(
            columns={'unit': 'ask_unit'})
    ...
    def get_new_quote_data(self, message, df_quote):
        ...
        df_quote = pd.concat([df_quote, df_quote2], axis=0).drop_duplicates(
            subset='price', keep='last')
        df_quote = df_quote.sort_values(
    ...
    def update_quote_data(self, input_symbol):
        ...
        try:
            if input_symbol == symbol:
                df_quote, price_list, symbol = self.get_new_quote_data(
                    quote_msg, df_quote)
            else:
                time.sleep(1)
                df_quote, price_list, symbol = self.get_first_quote_data(
                    quote_msg)
        except:
            time.sleep(1)
        ...
        for i in range(len(dataframe)):
            ...
            for col in dataframe.columns:
                ...
                if col == 'price':
                    ...
                    cell = html.Td(children=value, style={
                                   'font-size': 16, 'text-align': 'center'})
